...
def _extract_case_id(from_obj: Any) -> str:
    s = str(from_obj)
    m = re.search(r'([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})', s, flags=re.I)
...
def _extract_reason(agent_result: Any) -> str:
    m = re.search(r"'reason':\s*'([^']*)'|\"reason\":\s*\"([^\"]*)\"", str(agent_result))
...
class ThoughtCapture(BaseCallbackHandler):
    ...
    def on_agent_action(self, action, **kwargs):
        ...
        logger.info("[AgentThought] Tool=%s | Input=%s", rec["tool"], _truncate(rec["tool_input"]))
...
#     "피해자/시나리오를 바탕으로 ➊시뮬 실행(MCP), ➋판정/근거, ➌지침 조회/삽입(다음 라운드), ➍예방책 저장을 반복합니다. "
...
REACT_SYS = (
    ...
    "       └ 매핑 규칙: 직전 admin.judge 결과에서 phishing==true → 'P'(Protect), phishing==false → 'A'(Attack)\n"
...
def run_orchestrated(db: Session, payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        for round_no in range(1, max_rounds + 1):
            # ── (A) 시뮬레이션 실행 (LLM 경유, case_id는 우리가 직접 관리/주입) ──────────────
            ...
            if round_no == 1:
                ...
                if not case_id:
                    logger.error("[CaseID] 라운드1 case_id 추출 실패 | res_run=%s", _truncate(res_run))
            ...
            # ── (B) 판정 ────────────────────────────────────────
            res_judge = ex.invoke({
                "input": "admin.judge 호출.\n" + json.dumps({"data": {"case_id": case_id, "run_no": round_no}}, ensure_ascii=False)
            }, callbacks=[cap])
            used_tools.append("admin.judge")
            phishing = _extract_phishing(res_judge)
            reason = _extract_reason(res_judge)
            logger.info("[Judgement] round=%s | phishing=%s | reason=%s", round_no, phishing, reason)
            ...
            save_payload = {
                ...
                "victim_id": victim_id,
                "run_no": round_no,
                "summary": f"Round {round_no} judgement: {'PHISHING' if phishing else 'NOT PHISHING'}. "
            ...
            if round_no < max_rounds:
                ...
                # ✅ 지침 텍스트 추출 + 비어있을 때 안전한 기본값
                guidance_text = _extract_guidance_text(res_pick) or ""
                if not guidance_text:
                    logger.warning("[GuidancePicked] empty guidance_text; fallback to generic")
                    guidance_text = "기본 예방 수칙을 따르세요."

                logger.info("[GuidancePicked] round=%s | kind=%s | text=%s",
                            round_no, guidance_kind, _truncate(guidance_text, 300))
...
# # ─────────────────────────────────────────────────────────
# # ReAct 시스템 프롬프트 (강한 레일가드)
# # ─────────────────────────────────────────────────────────
# REACT_SYS = (
#     "당신은 보이스피싱 시뮬레이션 오케스트레이터입니다.\n"
#     "반드시 도구만 사용해 작업하세요. (설명만 하거나 직접 결과를 쓰지 말 것)\n"
#     "사이클 규칙:\n"
#     "  • 1라운드: sim.fetch_entities → sim.compose_prompts(지침 금지) → mcp.simulator_run\n"
#     "  • 판정: admin.judge → phishing/이유 추출\n"
#     "  • 예방책: 매 라운드 admin.save_prevention 저장(요약/스텝)\n"
#     "  • 2라운드~: admin.pick_guidance → mcp.simulator_run(guidance와 함께, case_id_override/round_no 반드시 전달)\n"
#     "제약:\n"
#     "  • 1라운드엔 guidance를 어느 도구에도 넣지 마세요.\n"
#     "  • 2라운드부터 guidance는 오직 mcp.simulator_run.arguments.guidance로만 주입.\n"
#     "  • 항상 동일 case_id로 이어가기: 1라운드에서 받은 case_id를 2라운드부터 case_id_override로 넣고 round_no도 함께 넣기.\n"
#     "  • prevention은 매 라운드 저장.\n"
#     "출력 포맷:\n"
#     "  Thought → Action → Action Input(JSON; 최상위키 \"data\") → Observation… 반복, 마지막에 Final Answer."
# )
...
#                     sim_payload["guidance"] = {"type": guidance_kind, "text": guidance_text}
#             else:
#                 # 라운드1: 어떤 경우에도 guidance가 섞이지 않도록 세이프가드
#                 sim_payload.pop("guidance", None)
...
#                 }, callbacks=[cap])
#                 used_tools.append("admin.pick_guidance")
#                 guidance_text = _extract_guidance_text(res_pick)
#                 logger.info("[GuidancePicked] round=%s | kind=%s | text=%s",
#                             round_no, guidance_kind, _truncate(guidance_text, 300))
