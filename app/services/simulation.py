...
#     r"(지점\s*방문|직접\s*방문|내가\s*직접\s*가서)",
#     r"(신고|경찰|사이버수사대|금융감독원|금감원)",
#     r"(보이스\s*피싱|사기|수상|수상\s*하다|이상\s*하다)",
#     r"(거부|못\s*합니다|안\s*합니다|않겠습니다|응하지\s*않)",
#     r"(끊[을게요|겠습니다]?|통화\s*그만|여기서\s*마무리하겠습니다)",
# ]
# DEFENSE_REGEXES  = [re.compile(p, re.IGNORECASE)  for p in DEFENSE_KEYWORDS]

# def _victim_defended(text: str) -> tuple[bool, list[str]]:
#     """피해자가 방어 의사를 명확히 표명했는지 판정하고, 매칭된 키워드를 반환."""
#     t = (text or "").strip()
#     if not t:
#         return False, []
#     hits = []
...
#             case.evidence = "정형 룰 기반 추정: 피해 확정 키워드 감지"
#             db.commit()
#             break
        
#         defended, hits = _victim_defended(victim_msg.content)
#         if defended:
#             case.phishing = False
#             case.status = "completed"
#             case.completed_at = datetime.now(timezone.utc)
#             case.defense_count = exchange_count            # ✅ CHANGED: ‘막을 때까지의 교환 수’
#             # 간단한 근거 기록
#             if hits:
#                 case.evidence = f"정형 룰 기반 추정: 방어 키워드 감지 → {', '.join(hits[:5])}"
#             else:
#                 case.evidence = "정형 룰 기반 추정: 방어 키워드 감지"
#             db.commit()
#             break
