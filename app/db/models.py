...
class PersonalizedPrevention(Base):
    ...
    __table_args__ = (Index("ix_pp_case_run_victim", "case_id", "run",
                            "victim_id"), )
class CustomVictim(Base):
    __tablename__ = "custom_victim"

    id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
    name: Mapped[str] = mapped_column(String(100), nullable=False)

    # 자유형 JSON 필드
    meta: Mapped[dict] = mapped_column(JSONB, default=dict)           # { age, education, gender, address }
    knowledge: Mapped[dict] = mapped_column(JSONB, default=dict)      # { comparative_notes: [], competencies: [] }
    traits: Mapped[dict] = mapped_column(JSONB, default=dict)         # { ocean: {...}, vulnerability_notes: [] }
...
class CustomScenario(Base):
    ...
    # 사용자 입력은 purpose만 받고, steps는 서버(에이전트/헬퍼)가 생성
    purpose: Mapped[str] = mapped_column(String(255), nullable=False)
    steps: Mapped[list] = mapped_column(JSONB, default=list)          # ["...", "...", ...]
    source: Mapped[dict | None] = mapped_column(JSONB, default=dict)  # tavily 결과 요약 등
